...
import { STAGES, bodyComplete, goalComplete, safetyComplete } from "./lib/flow"
...
export default function App() {
  ...
  const profile: Profile | null = useMemo(() => {
    if (!bodyComplete(sex, values)) return null
    return {
  ...
  /**
   * The figure has to be alive from the first frame, so it draws from a
   * starting body until each reading replaces it. Nothing unset is reported as
   * measured anywhere: the rail shows a dash, the strip under the figure leaves
   * it out, and the flow does not move until every reading is set.
   */
  ...
  function changeSex(next: Sex) {
    setSex(next)
    // Pregnancy is only asked of women, so a man cannot be left holding a yes.
    if (next === "male") setFlags((prev) => prev.filter((f) => f !== "pregnant"))
  }
